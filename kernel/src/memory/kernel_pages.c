 ...
 *
 * Pages are processed backwards so the most recently installed
...
)
{
    while (mapped_pages > 0) {
        ...
        if (vmm_unmap_page(
            ...
            /*
             * This frame was allocated by the transaction, so
             * returning it should always succeed.
             */
            (void)pmm_free_page(
                physical_address
            );
...
)
{
    ...
    /*
     * These bounds also make later page-to-byte conversions safe.
     */
    if (reservation_pages >
            UINT64_MAX /
                KRISHNA_PAGE_SIZE ||
        page_count >
            SIZE_MAX /
    ...
    /*
     * Pages must temporarily be writable so stale frame contents
     * can be cleared before the final permissions are installed.
     */
    ...
    /*
     * A kernel allocation must not expose information left behind
     * in a previously used physical frame.
     */
    memset(
    ...
    /*
     * Restore read-only permissions when writable access was not
    ...
    /*
     * Publish the descriptor only after the complete transaction
     * succeeds. Therefore a failed request cannot partially modify
     * the caller's output.
     */
    *allocation =
...
)
{
    ...
    if (allocation->reservation_base >
            UINT64_MAX -
                guard_prefix_bytes ||
    ...
    /*
     * Guard pages must still be unmapped. If one unexpectedly has
     * a translation, releasing its virtual reservation would make
     * that live mapping available for reuse.
     */
    for (size_t page = 0;
         page <
            allocation->guard_pages_before;
    ...
    /*
     * Validate the complete mapping before modifying anything.
     * This rejects stale, forged and double-release descriptors.
     */
    for (size_t page = 0;
         page <
            allocation->mapped_pages;
    ...
    /*
     * Unmap in reverse order. vmm_unmap_page() also reclaims empty
     * intermediate page tables owned by the VMM.
     */
    for (size_t page =
             allocation->mapped_pages;
         page > 0;
...
bool kernel_pages_self_test(void)
{
    ...
    /*
     * Verify that a failed transaction leaves the output descriptor
    ...
    /*
     * Allocate three pages from a four-page-aligned virtual address.
     */
    bool allocated =
        kernel_pages_allocate(
            3,
    ...
    bool cleanup_passed =
        released &&
        allocation.reservation_base == 0 &&
        allocation.mapped_base == 0 &&
        allocation.mapped_pages == 0 &&
        before.free_pages ==
            after.free_pages &&
